...
class World {
  ...
  join (worldId, userId) {
    ...
    if (!state.players[PLAYER_PEKORA]) {
      if (state.players[PLAYER_BAIKINKUN] === userId) throw new Error('same name user joined exception')
      state.players[PLAYER_PEKORA] = userId
      return PLAYER_PEKORA
    }
    if (state.players[PLAYER_PEKORA] === userId) throw new Error('same name user joined exception')
